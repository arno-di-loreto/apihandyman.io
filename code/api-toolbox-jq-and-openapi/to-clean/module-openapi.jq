  ...
  map ( # Applies a transformation to each element
    ...
    map( # Applies a transformation to each element
      ...
      # Creates a new JSON object
      {
        ...
        original: .value # The original operation's data
      }
    )[] # Flattens array to avoid having an array 
        # of array of {path, method, summary}
  ) # Now we have an array of {path, method, summary, original}
; # listoperations function's end

def oas_operations_to_text: # Prints operations as raw text
  map( # Applies a transformation to each element
    .method + "\t" + 
    .path +"\t" +
    .summary + 
